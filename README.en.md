...
## Chinese filenames & encoding (Windows)

- The entry scripts (`run.bat` / the command generated by `install.ps1` / `green.bat`)
  contain `chcp 65001 >nul`, switching the console codepage to UTF-8. This is what
  makes programs like ffmpeg handle Chinese filenames without `Illegal byte sequence`
  on Simplified-Chinese Windows (verified: without it, Chinese filenames fail to open).
- chcp runs once at startup; it may cause a one-time terminal "redraw" (looks like a
  clear screen), which is normal and does not affect usage.
