...
4. **每轮用量统计**:显示本次消耗的输入/输出 token、**输入构成**(提示词注入 + 上下文累计 +
   本轮新输入,估算)与缓存命中(`SHOW_USAGE` 开关,`--no-usage` 可关)。
   - 示例:`输入 29188 ≈ 提示词 379 + 上下文 26300 + 本轮新 2508(估算)+ 输出 328(3 次请求,含工具循环)| 缓存命中 25344 (87%) | 未命中 3844`
   - 解释:输入是**本轮所有 API 请求的累计**(工具循环/重试每次都带全部历史),
     因此单轮输入 = 单次请求输入 × 请求次数;真正按全价计费的是"未命中"部分
     (命中部分按约 1/60 价,所以缓存命中率高时实际成本很低)。
